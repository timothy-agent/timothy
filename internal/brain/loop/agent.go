...
type Request struct {
	...
	// base tool. Missions set it on the planner call and on every turn
	// of a mission with a tool_allowlist (see runner.go). Chat's
	// agent-authored allowlist (empty = no tools to the user) is
	// resolved to a non-empty list before it ever reaches here (see
	// chat.resolveToolAllow).
	ToolAllow []string
	ModelHint string
	System    string
