...
func TestIsActionableTransition(t *testing.T) {
	cases := []struct {
		...
		{"working to done is NOT actionable (NotifyConsumer handles terminal states)", StatusWorking, StatusDone, false, ""},
		{"working to error is NOT actionable (NotifyConsumer handles terminal states)", StatusWorking, StatusError, false, ""},
