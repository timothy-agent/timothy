...
// pendingAlter returns the SQL block recorded under issue in
// scripts/pending-alters.md.
func pendingAlter(t *testing.T, issue string) string {
	t.Helper()
	raw, err := os.ReadFile("../../../scripts/pending-alters.md")
	if err != nil {
		t.Fatal(err)
	}
	s := string(raw)
	i := strings.Index(s, "Issue "+issue+":")
	if i < 0 {
		t.Fatalf("no entry for issue %s", issue)
	}
	s = s[i:]
	start := strings.Index(s, "```sql\n")
	end := strings.Index(s[start+7:], "```")
	if start < 0 || end < 0 {
		t.Fatalf("no sql block for issue %s", issue)
	}
	return s[start+7 : start+7+end]
}

// TestTelegramDestinationMigration runs issue #831's pending alter on a
// database that still has the old kind constraint and two telegram
// destinations: two disabled channels, two channel destinations, and
// the new constraint.
func TestTelegramDestinationMigration(t *testing.T) {
	pool := channelTestPool(t)
	ctx := t.Context()
	db, _ := pool.Get()
	tag := marker + "mig-" + time.Now().Format("150405.000000") + "-"
	...
	var others int
	if err := db.QueryRow(ctx, `SELECT count(*) FROM destinations WHERE kind NOT IN ('email', 'webhook', 'channel', 'github', 'bitbucket')`).Scan(&others); err != nil || others != 0 {
		t.Fatalf("foreign rows = %d %v", others, err)
	}
	t.Cleanup(func() {
		cctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
		_, _ = db.Exec(cctx, `DELETE FROM destinations WHERE name LIKE $1 || '%'`, tag)
		_, _ = db.Exec(cctx, `ALTER TABLE destinations DROP CONSTRAINT IF EXISTS destinations_kind_check`)
		_, _ = db.Exec(cctx, `ALTER TABLE destinations ADD CONSTRAINT destinations_kind_check CHECK (kind IN ('email', 'webhook', 'channel', 'github', 'bitbucket'))`)
	})
	exec(`ALTER TABLE destinations DROP CONSTRAINT destinations_kind_check`)
	exec(`ALTER TABLE destinations ADD CONSTRAINT destinations_kind_check CHECK (kind IN ('email', 'webhook', 'telegram', 'github', 'bitbucket'))`)
	exec(`INSERT INTO destinations (name, kind, config, credential_ref) VALUES ($1, 'telegram', '{"chat_id":"-100"}', 'TG_A'), ($2, 'telegram', '{"chat_id":"42"}', 'TG_B')`,
		tag+"alerts", tag+"ops")
	// A clash with the derived name gets a number.
	exec(`INSERT INTO channels (name, kind, credential_ref) VALUES ($1, 'telegram', 'TG_X')`, tag+"alerts (bot)")

	exec(pendingAlter(t, "#831"))

	rows, err := db.Query(ctx, `SELECT d.name, d.kind, d.credential_ref, d.config->>'chat_id', c.name, c.kind, c.credential_ref, c.enabled
		FROM destinations d JOIN channels c ON c.id::text = d.config->>'channel_id'
		WHERE d.name LIKE $1 || '%' ORDER BY d.name`, tag)
	if err != nil {
		t.Fatal(err)
	}
	type moved struct {
		dest, kind, ref, chat, channel, channelKind, channelRef string
		enabled                                                 bool
	}
	var got []moved
	for rows.Next() {
		var m moved
		if err := rows.Scan(&m.dest, &m.kind, &m.ref, &m.chat, &m.channel, &m.channelKind, &m.channelRef, &m.enabled); err != nil {
			t.Fatal(err)
		}
		got = append(got, m)
	}
	rows.Close()
	want := []moved{
		{tag + "alerts", "channel", "", "-100", tag + "alerts (bot 2)", "telegram", "TG_A", false},
		{tag + "ops", "channel", "", "42", tag + "ops (bot)", "telegram", "TG_B", false},
	}
	if len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("migrated = %+v, want %+v", got, want)
	}
	if _, err := db.Exec(ctx, `INSERT INTO destinations (name, kind) VALUES ($1, 'telegram')`, tag+"again"); err == nil {
		t.Fatal("the new constraint accepts telegram")
	}
	exec(`INSERT INTO destinations (name, kind) VALUES ($1, 'channel')`, tag+"chan")
}
