...
export function PipelineCard({
  ...
  return (
    <Card
      ...
      density="operational"
      className={`w-full select-none space-y-2.5 transition ${
    ...
    >
      ...
      <div className="flex items-center gap-1">
        {serving && <Badge variant="good">serving</Badge>}
        {!usable && status?.skip_reason && (
          <span className="truncate text-xs text-warning" title={status.skip_reason}>
            {status.skip_reason}
          </span>
        )}
        <span className="ml-auto flex items-center gap-1">
          ...
          <IconButton type="button" label={`Remove ${model}`} icon={Trash2} size="xs" onClick={onRemove} />
        </span>
      </div>
    </Card>
  )
