...
- When the rules name a required sponsor technology, fetch its documentation with fetch_url and add a checklist row quoting the languages and runtimes it supports. No idea names a stack before that row exists.
...
- Research each surviving candidate against what already exists: products, GitHub, papers, articles, prior entries to this event, sponsor examples. Name the closest existing solution, what it already solves, and what this idea does differently. A candidate with no named comparison has not been researched, and a comparison recalled from memory is not research: each named solution comes from a search_web result or a fetch_url page retrieved in this mission, and carries that URL next to its name. The harness checks every URL in the artifact against the pages fetched and the search results returned this turn; a URL it never saw fails the unit, so a comparison from memory cannot pass.
...
**Differentiation:** closest existing solution with its URL, what it already solves, what this does differently, and the rating in words.
...
- Ideas are for this operator, not a generic entrant. Use what search_memory and the goal say about them, their languages and stack, the infrastructure and data they already run, and the tasks they actually repeat. When a required sponsor technology rules out a standing preference, say which preference yields and why. A Build line names only languages the sponsor SDK row lists; a preferred language the SDK does not support yields, and the Build line says so.
...
- Stop after the ideas. The build starts only after the operator picks one. When the mission has a followup_mission tool, offer to open the build mission through it with `attach` set to `rules-checklist.md` and `ideas.md`, and a `brief` whose objective is the chosen idea's pitch and whose scope is its Build line. That brief's acceptance criteria are the checklist rows the build must satisfy plus the submission-pack units.
...
| "I already know the closest products, no need to search"       | Memory of a market is not a search of it. Run search_web per survivor and name the result that was read.              |
...
- A closest existing solution has no URL, or a URL that was not fetched or returned by search_web in this mission.
...
- `ideas.md` is being written and search_web was offered but never called.
- A Build line names a language the sponsor SDK row does not list, with no note saying which preference yielded.
